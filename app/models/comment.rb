class Comment < ActiveRecord::Base
  ...
  attr_accessible :body, :subject, :author_name, :obj_type, :obj_id, :bot_email, :contacts
  ...
  def comments
    @comments ||= ( (@sub_comments || (self.obj_id && self.obj_type != 'Comment')) ? 
       ...
       Comment.find(:all, :conditions => ['obj_id = ? AND obj_type = "Comment"', self.id], :order => 'created_at DESC')
    )
  end
  ...
  def sub_comments(since=nil)
    @sub_comments ||= (
      cmts = (since ? 
                Comment.find(:all, :conditions=>["created_at > ? AND root_id = ?", since, self.id]) :
