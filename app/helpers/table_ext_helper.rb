module TableExtHelper
  ...
  module TableBuilder
    ...
    def column_title(name, title, p = {}, options = {})
      p = p.to_unsafe_h if p.present? && p.respond_to?(:to_unsafe_h)
      name = name.to_s
      res = '<div align="center">'
      full_title = options[:title] || ''
      full_title = "#{full_title}, " unless full_title.blank?
      stop_sorting = ((p[:sort] == name) ?
                        raw(link_to(icon_close, params.to_unsafe_h.except(:sort, :sort_dir),
      ...
      if p[name]
        ...
        res << raw(content_tag('div',
                               link_to(raw(value + '&nbsp;' + icon_close),
                                       params.to_unsafe_h.except(name, :page),
    ...
    def column_title_tag(tag, name, title, p = {}, html_options = {})
      ...
      raw(content_tag(tag,
                      raw(column_title(name, title.html_safe, p, html_options)),
