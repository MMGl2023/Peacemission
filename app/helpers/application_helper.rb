...
module ApplicationHelper
  ...
  def expandable_block(text='&gt;&gt;', options={}, &block)
    ...
    closing_link = link_to_function(
      options[:icon_close] || (respond_to?(:icon_close) && icon_close) || '&Lt;', 
      closing_javascript,
    ...
    unless expanded
      concat("<script>$('#{o_id}').hide();</script>", block.binding)
    else
      unless options[:noshrink]
        concat("<script>$('#{c_id}').hide();</script>", block.binding)
      end
    end
  end

  def multiple_link_to(ary, sep, *args)
    ary.map{|a| link_to(a, *args)}.join(sep)
  end
    ...
    def define_icon(*names)
      names.each do |name|
        ...
        define_method(meth = 'icon_' + name.to_s.gsub('-', '_')) do |*args|
          ...
          image_tag(icon_src, {:class => 'icon'}.merge!(opts))
        end
