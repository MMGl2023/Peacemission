class LostsController < ApplicationController
  ...
  before_action :signin_if_not_yet, except: [:show, :index, :search, :update, :create, :new, :edit]
  ...
  def allow_edit?
    has_permission?(:losts) || session.id == @lost.session_id
  end
  ...
  def update_attributes
    @permitted_params = (params.require(:lost).permit!).to_hash
    extract_fuzzy_date(:birth_date, @permitted_params)
    extract_fuzzy_date(:lost_on, @permitted_params)
    extract_fuzzy_date(:found_on, @permitted_params)

    @lost.attributes = @permitted_params
    # .project(
  ...
  def update_was_successful
    flash[:info] = ("Запись на '#{@lost.full_name}' успешно обновлена. <a href=\"#{edit_lost_path(@lost)}\">Редактировать</a>").html_safe
  ...
  def list
    ...
    params[:active] = true unless params[:show_all] == 'true' || action_name == 'list'
    ...
    @losts = Lost.where(cnds.delete(:conditions)).order(cnds.delete(:order)).paginate(cnds)
