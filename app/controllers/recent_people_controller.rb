class RecentPeopleController < ApplicationController
  ...
  before_action :signin_if_not_yet, except: ['recent', 'show', 'list', 'search', 'index', 'db_info']
  ...
  before_action :extract_fuzzy_dates, only: [:create, :update]

  before_action :check_authority, only: [:update, :destroy]

  before_action :check_create_authority, only: [:create]
  ...
  # GET /people/dump.xml
  def dump
    @recent_people = RecentPerson.all
    respond_to do |format|
  ...
  # GET /people/new.xml
  def new
    @recent_person = RecentPerson.new(params.require(:recent_person).permit!)
  ...
  def extract_fuzzy_dates
    @permitted_params = (params.require(:recent_person).permit!).to_hash
  ...
  def create
    if params[:cancel]
      ...
      @recent_person = RecentPerson.new(@permitted_params)
      # init_lost_id
      # check_requests
      respond_to do |format|
  ...
  def update
    ...
    if params[:cancel]
      ...
      respond_to do |format|
        ...
        if @recent_person.update(@permitted_params)
  ...
  def list
    ...
    @recent_people = RecentPerson.where(cnds.delete(:conditions)).order(cnds.delete(:order)).paginate(cnds)
