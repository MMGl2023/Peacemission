class UsersController < ApplicationController
  ...
  before_filter :find_user_or_current_or_access_denied, :only => [:show, :edit, :update_image, :update]

  before_filter :find_user_or_bad_request, :only => change_state_actions
  
  # before_filter :not_authenticated_required, :only => [:create] 
  
  before_filter :authentication_required, :only => [:home]
  
  
  require_permission :users, :only => [:index, :destroy] + change_state_actions
    
  before_filter :can_modify_user?, :only => [:update, :edit, :update_image]
  ...
  def update
    ...
    if params[:user][:password].length > 0
      if User.authenticate(@user.login, params[:user].delete[:current_password])
