module AutoCompleteExt
  ...
  module ControllerClassMethods
    ...
    def ext_auto_complete_for(obj_name, attribute, options = {}, &block)
      ...
      # we need before_filter method to convert texts (describing objects for
      ...
      if extract_method
        ...
        before_filter filter_method, :only => extract_before
      end
    end
  end
    ...
    def auto_complete_fields_for_many(obj, assoc_name, options={})
      ...
      content_tag('ul',
        (0...n).map{|i|
          ...
          prev_value = values[i]
          show_next = (i==n-1) ? "" : "$('#{li_next_id}').show()" 
          content_tag('li',
            text_field_with_auto_complete(prefix, item,
              { 
                :width => 16, :value => values[i] || '',
                :onclick => show_next
              }.merge!(options[:html]||{}),
              { 
                :url => { :action => "auto_complete_for_#{prefix}_#{name}", :name=>item } #,
